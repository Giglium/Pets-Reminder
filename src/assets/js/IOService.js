...
export default class IOService {
  ...
  scheduleNotificationForPreciseInstant(id, month, day, hour, minute, text, repeat, birthday) {
    ...
    if (repeat) {
      if (birthday) {
        schedule.trigger = {every: {month: month, day: day, hour: hour, minute: minute}, count: 365};
      } else {
        schedule.trigger = {every: {hour: hour, minute: minute}, count: 365};
      }
    } else {
      schedule.trigger = {every: {month: month, day: day, hour: hour, minute: minute}, count: 1};
    }
