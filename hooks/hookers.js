module.exports = function (ctx) {
  ...
  const sys = {
    ...
    deleteFolderRecursive (path, doNotDeleteSelf = false) {
      if (fs.existsSync(path)) {
        fs.readdirSync(path).forEach((file) => {
          ...
          if (fs.lstatSync(curPath).isDirectory()) { sys.deleteFolderRecursive(curPath) } else { fs.unlinkSync(curPath) }
        })
    ...
    makeDevServerChanges () {
      ...
      if (conf('allow-navigation').length === 0) { conf('widget').append('<allow-navigation href="*" />') } else {
        const target = conf('allow-navigation')
